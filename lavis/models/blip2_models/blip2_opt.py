...
class Blip2OPT(Blip2Base):
    ...
    def generate_verbose_images(self, samples):        
        ...
        for i, index in enumerate(early_exit_layers):
            tmp_ent = entropy_from_logits(dict_outputs[index].squeeze())[-1]
            if tmp_ent > mini_ent:
                mini_ent = tmp_ent
                mini_ind = index
        logits_mini = hidden_dict[mini_ind].squeeze()
...
###########Verbose attack's loss functions##############
        ...
        loss_eos = 0 * logits[:,self.eos_token_id].mean()
        ...
        return {"loss1": loss_s_r, "loss2": loss_eos, "loss3": loss_diversity}
